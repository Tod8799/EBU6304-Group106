...
const toggleMemberListing = "在简短列表视图和详细列表视图之间切换";

const sortLexicalLabel = "按字典顺序对成员详细信息进行排序";
const sortSourceLabel  = "按源顺序对成员详细信息进行排序";
...
document.addEventListener("DOMContentLoaded", function(e) {
    setTopMargin();
    // Inject consistent project title into the top navigation and document title
    (function insertProjectTitle(){
        try {
            var projectName = 'BUPT TA Recruitment and Management System';
            // prefix document title for clarity
            if (document.title && document.title.indexOf(projectName) !== 0) {
                document.title = projectName + (document.title ? ' - ' + document.title : '');
            }
            var navs = document.querySelectorAll('div.nav-content');
            for (var i = 0; i < navs.length; i++) {
                var nav = navs[i];
                if (!nav.querySelector('.project-title')) {
                    var a = document.createElement('a');
                    a.className = 'project-title';
                    a.href = pathtoroot + 'index.html';
                    a.textContent = projectName;
                    a.style.cssText = 'font-weight:600;margin-right:1rem;color:inherit;text-decoration:none;';
                    // insert after skip-nav if present, otherwise prepend
                    var skip = nav.querySelector('.skip-nav');
            ...
            // fail silently to avoid breaking docs UI
            console.error('insertProjectTitle error', err);
        }
    })();
    // Translate common Chinese UI strings to English at runtime
    (function translateUiToEnglish(){
        try {
            ...
            var map = {
                ...
                '您的浏览器已禁用 JavaScript。': 'JavaScript is disabled in your browser.',
                '程序包': 'Package',
                '类': 'Class',
                '成员': 'Members',
                '程序包列表': 'Package List',
                '未命名程序包': 'Unnamed package',
                '选择:': 'Select:',
                '确定': 'OK',
                '重置': 'Reset'
            };
            ...
            function replaceText(node) {
                if (node.nodeType === Node.TEXT_NODE) {
                    ...
                    // replace attributes that commonly contain labels
                    ['title','aria-label','placeholder','value','alt'].forEach(function(attr){
                        if (node.hasAttribute && node.hasAttribute(attr)) {
                            var v = node.getAttribute(attr);
                            Object.keys(map).forEach(function(k){ if (v && v.indexOf(k)!==-1) v = v.split(k).join(map[k]); });
            ...
            console.error('translateUiToEnglish error', err);
        }
    })();
